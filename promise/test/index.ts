...
describe('Promise', () => {
  ...
  it('new Promise(fn) 中的 fn 立即执行', () => {
    let fn = sinon.fake();  // fake()假函数
    new Promise(fn);
    assert(fn.called);
    // 判断 fn 立即执行-比较挫的写法
    // let called = false;
    // const promise = new Promise(() => {called = true;});
    // // @ts-ignore
    // assert(called === true);
  });
  ...
  it('2.2.2 如果onFulfilled是函数', done => {
    ...
    const promise = new Promise(resolve => {
      ...
      setTimeout(() => {
        ...
        assert.isTrue(succeed.calledOnce); // calledOnce 保证只调用一次
        assert(succeed.calledWith(233)); // calledWith预测函数传参
        done();
      }, 0);
    });
